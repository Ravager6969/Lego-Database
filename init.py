...
information = ['Created on: Augest 7, 2020', 'Version: 1.0', 'Lines of Code: ~ 1600']
...
class club_display(object):
    ...
    def create_club_display(self):
        ...
        description = club_description(self.club_choice_display)

        description_display = golem(description, sky_blue, (width/2, height/2), font)
        description_display.KINGS()
        ...
        members = golem(leader.split('_')[0], gold, (width-400, 400), font)
        members.KING()

        member_id = golem((leader.split('_')[1]).split(' ')[0], gold, (width-500, 400), font)
        member_id.KING()
        ...
        for x in range(len(wolfking[list_clubs[self.club_choice_display]])):
            ...
            members = golem(wolfking[list_clubs[self.club_choice_display]][y].split('_')[0], sky_blue, (width-400, 400 + x*100+100), font)
            members.KING()

            member_id = golem(wolfking[list_clubs[self.club_choice_display]][y].split('_')[1], sky_blue, (width-500, 400 + x*100+100), font)
            member_id.KING()
